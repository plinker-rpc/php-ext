...
ZEPHIR_INIT_CLASS(Plinker_Lib_Certificate) {
	...
	ZEPHIR_REGISTER_CLASS(Plinker\\Lib, Certificate, plinker, lib_certificate, plinker_lib_certificate_method_entry, ZEND_ACC_FINAL_CLASS);

	zend_declare_property_null(plinker_lib_certificate_ce, SL("cert_path"), ZEND_ACC_PRIVATE TSRMLS_CC);
...
PHP_METHOD(Plinker_Lib_Certificate, generate) {
	...
	ZVAL_LONG(&_0, 275);
	ZEPHIR_CALL_FUNCTION(&_1, "filter_var", NULL, 6, &ip, &_0);
	zephir_check_call_status();
	if (!(zephir_is_true(&_1))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(spl_ce_InvalidArgumentException, "Parameter must be a valid IP address.", "plinker/lib/certificate.zep", 43);
		return;
	}
	ZEPHIR_INIT_VAR(&_2);
	...
	ZEPHIR_CALL_FUNCTION(&privkey, "openssl_pkey_new", NULL, 7, &_2);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_3);
	...
	ZEPHIR_CALL_FUNCTION(&cert, "openssl_csr_new", NULL, 8, &_3, &privkey);
	ZEPHIR_UNREF(&privkey);
	zephir_check_call_status();
	ZVAL_LONG(&_0, 1825);
	ZEPHIR_CALL_FUNCTION(&_4, "openssl_csr_sign", NULL, 9, &cert, &__$null, &privkey, &_0);
	zephir_check_call_status();
	ZEPHIR_CPY_WRT(&cert, &_4);
	ZEPHIR_MAKE_REF(&certString);
	ZEPHIR_CALL_FUNCTION(NULL, "openssl_x509_export", NULL, 10, &cert, &certString);
	ZEPHIR_UNREF(&certString);
	zephir_check_call_status();
	ZEPHIR_MAKE_REF(&privkeyString);
	ZEPHIR_CALL_FUNCTION(NULL, "openssl_pkey_export", NULL, 11, &privkey, &privkeyString);
	...
	ZEPHIR_CALL_FUNCTION(NULL, "openssl_pkcs12_export", NULL, 12, &certString, &p12String, &privkeyString, &_6, &_5);
	ZEPHIR_UNREF(&p12String);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&pemString);
	ZEPHIR_CONCAT_VV(&pemString, &certString, &privkeyString);
	ZEPHIR_CALL_FUNCTION(&_7, "sha1", NULL, 13, &pemString);
	zephir_check_call_status();
	...
	if (!((zephir_file_exists(&cert_path TSRMLS_CC) == SUCCESS))) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "mkdir", NULL, 14, &cert_path, &_10$$4, &__$true);
		zephir_check_call_status();
	}
	ZEPHIR_INIT_VAR(&_11);
	...
	ZEPHIR_CALL_FUNCTION(&_15, "date_create", NULL, 15);
	zephir_check_call_status();
